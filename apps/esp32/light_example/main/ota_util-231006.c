...
//#define CONFIG_OTA_SERVER_URL "https://192.168.1.89:4443/"
#define CONFIG_OTA_SERVER_URL "https://otaserver.ddns.net:3443/"
#define CONFIG_FIRMWARE_VERSOIN_INFO_URL CONFIG_OTA_SERVER_URL"version_info.json"
#define CONFIG_FIRMWARE_UPGRADE_URL CONFIG_OTA_SERVER_URL"signed_light_example.bin"
...
static ota_err_t _get_available_version(char *update_info, unsigned int update_info_len, char *firmware_version, char **new_version)
{
    ...
    printf("update_info= %s,  update_info_len=%d,  firmware_version= %s", update_info,update_info_len,  firmware_version);
	...
	for (int i = 0 ; i < cJSON_GetArraySize(array) ; i++)
	{
        ...
        printf("upgrade = %s, firmware_version = %s\n", upgrade, firmware_version);                //시험        
    #if 0       //시험
...
static int _crypto_sha256(const unsigned char *src, size_t src_len, unsigned char *dst)
{
	...
	printf("src: %d@%p, dst: %p\n", src_len, src, dst);

	ret = mbedtls_sha256_ret(src, src_len, dst, 0);
	if (ret) {
		printf("mbedtls_sha256_ret = -0x%04X\n", -ret);
		return ret;
	}
...
static bool _check_firmware_validation(const unsigned char *sha256, unsigned char *sig_data, unsigned int sig_len)
{
	...
	if (sig_data[0] != 0xaa || sig_data[1] != 0x55 ||
		sig_data[2] != 0xaa || sig_data[3] != 0x55) {
		printf("invalid signature header \n");
        printf("sig_data[0] = %x, sig_data[1] = %x, sig_data[2] = %x,sig_data[3] != %x\n", sig_data[0] ,sig_data[1],sig_data[2],sig_data[3]);
		goto clean_up;
	}

	// check signature's footer
	if (sig_data[OTA_DEFAULT_SIGNATURE_BUF_SIZE - 1] != 0x55 || sig_data[OTA_DEFAULT_SIGNATURE_BUF_SIZE - 2] != 0xaa ||
		sig_data[OTA_DEFAULT_SIGNATURE_BUF_SIZE - 3] != 0x55 || sig_data[OTA_DEFAULT_SIGNATURE_BUF_SIZE - 4] != 0xaa) {
        printf("sig_data[0] = %x, sig_data[1] = %x, sig_data[2] = %x,sig_data[3] != %x\n", sig_data[0] ,sig_data[1],sig_data[2],sig_data[3]);            
...
static ota_err_t _update_device()
{
	...
	while (1) {
		...
		if (data_read == 0) {
			printf("Connection closed,all data received - 1\n");
			break;
		}
		if (data_read < 0) {
...
static ota_err_t _read_version_info_from_server(char **version_info, unsigned int *version_info_len)
{
	...
	while (1) {
		int data_read = esp_http_client_read(client, upgrade_data_buf, OTA_DEFAULT_BUF_SIZE);
		if (data_read == 0) {
			printf("Connection closed,all data received - 2\n");
			break;
		}
		if (data_read < 0) {
			printf("Error: SSL data read error\n");
