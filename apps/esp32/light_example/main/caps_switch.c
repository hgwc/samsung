...
static void caps_switch_cmd_on_cb(IOT_CAP_HANDLE *handle, iot_cap_cmd_data_t *cmd_data, void *usr_data)
{
    ...
    if(msg[1]  != -1){
        ...
        if(main_queue != NULL)  {
            ...
            ESP_LOGI(TAG, "caps_switch_cmd_on_cb_xQueueSend: %d", msg[1]);
        }
    }
    //---
...
static void caps_switch_cmd_off_cb(IOT_CAP_HANDLE *handle, iot_cap_cmd_data_t *cmd_data, void *usr_data)
{
    ...
    // 센도리
    int8_t msg[2];    
    msg[1] = caps_switch_attr_switch_str2idx(value);
    if(msg[1]  != -1){
        msg[0] = 1;         // switch
        msg[1] = 0;
        if(main_queue != NULL)  {
            xQueueSend(main_queue, (void *)msg, (TickType_t)0);
            ESP_LOGI(TAG, "caps_switch_cmd_off_cb_xQueueSend: %d", msg[1]);
        }
    }
    //---
    printf("called [%s] func with num_args:%u\n", __func__, cmd_data->num_args);
